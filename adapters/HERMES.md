...
skill (all 14). After the loop it derives success from `hermes skills list` rather
...
override a `DANGEROUS` verdict. The script still attempts all 14 skills, verifies the
real installed set with `hermes skills list`, reports any missing names, and exits 1
when the verified count is incomplete. The other 12 stay on the default no-override
path.
...
- **GitHub API rate limit exhausted** -- `install-hermes.sh` makes 14 fetches per run
