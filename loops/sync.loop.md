...
never contend for the same worktree budget) via `.github/workflows/sync-opencode.yml`   |   local: weekly interval
...
close_out: dispatch the Memory Journalist to consolidate this session's substantial,
privacy-filtered observations into one session note, or log SKIP with a reason -- never
