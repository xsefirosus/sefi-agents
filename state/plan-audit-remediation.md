...
## Steps
- [x] 1. Soften Codex `approval_policy` from `never` to `on-failure`. (needs: -)
...
- [x] 2. Add the OpenCode and Hermes enforcement caveats to the README harness table. (needs: -)
...
- [x] 3. Extend `test-integration.sh` to exercise the cross-project memory mirror. (needs: -)
...
- [x] 4. Extend `test-integration.sh` to cover the `sync` loop spec. (needs: 3)
...
- [x] 5. Write `validate-comment-safety.sh` and wire it into `run-all.sh`. (needs: -)
...
- [x] 6. Write `.github/workflows/retro.yml` for the weekly-retro loop. (needs: -)
...
- [x] 7. Write `.github/workflows/sync.yml` for the sync loop. (needs: -)
...
- [x] 8. Uncomment the `morning-triage` schedule in `.github/workflows/triage.yml`. (needs: 6, 7)
...
- [x] 9. Point both weekly loop specs at their real workflow files. (needs: 6, 7)
...
- [x] 10. CHANGELOG entry and version bump to 0.3.29. (needs: 1, 2, 3, 4, 5, 8, 9)
  ...
  DONE, with one correction against this step's original plan: the owner asked mid-turn
  for 0.4.1, not 0.3.29 -- shipped as 0.4.1.
- [x] 11. Run `bash plugins/sefi-core/scripts/ci/run-all.sh` in full. (needs: 10)
