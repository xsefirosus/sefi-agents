...
for manifest in plugins/sefi-core/.claude-plugin/plugin.json plugins/sefi-core/.codex-plugin/plugin.json; do
  require_text "$manifest" '"version": "0.9.0"'
done
require_text .claude-plugin/marketplace.json '"version": "0.9.0"'
if [ "$(grep -Foc '"version": "0.9.0"' .claude-plugin/marketplace.json || true)" -ne 2 ]; then
  echo "FAIL: marketplace must carry v0.9.0 twice" >&2
  fail=1
fi
...
require_text Install.md 'v0.9.0'
