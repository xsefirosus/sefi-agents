#!/usr/bin/env bash
# budget-check.sh [--scope run|daily|dispatch] [--spent <usd>] [--config <path>]
...
# Float-safe comparison; exceeded when spent > cap.
if awk -v a="$spent" -v b="$CAP" 'BEGIN{exit !((a+0) > (b+0))}'; then
  echo "budget-check: EXCEEDED scope=$SCOPE spent=$spent cap=$CAP" >&2
  exit 1
fi
echo "budget-check: ok scope=$SCOPE spent=$spent cap=$CAP" >&2
exit 0
