...
| Every OpenCode subagent dispatch failed because a legacy harness-specific model alias could not resolve, live-observed on a real user install (not internal testing) | First-party | `adapters/OPENCODE.md`, `CHANGELOG.md` [0.2.2] | N/A -- already first-party. The installer now resolves explicit user maps or omits `model:` for the shipped flexible policy; regression coverage exercises both paths in `test-scripts.sh`. |
